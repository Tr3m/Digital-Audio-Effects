...
AllPassFilter::AllPassFilter(float _delayLength, float _feedback)
{
...
void AllPassFilter::setDelayLength(float newValue)
{
    //TODO: Add bounds check
...
float AllPassFilter::getDelayLength()
{
    return delayLength;
}

void AllPassFilter::setFeedback(float newValue)
{
    feedback = newValue;
}
...
void AllPassFilter::process(juce::AudioBuffer<float>& buffer, int numInputChannels, int numOutputChannels)
{
    ...
    for (int channel = 0; channel < numInputChannels; ++channel) {
        ...
        for (int i = 0; i < numSamples; ++i) {
            ...
            double delayRead = delayData[dpr];
            double comb = in + feedback*delayRead;
            out = -feedback*comb + delayRead;
...
float AllPassFilter::processSample(float sample, int channel)
{
    float* delayData = delayBuffer.getWritePointer(juce::jmin(channel, delayBuffer.getNumChannels() - 1));
